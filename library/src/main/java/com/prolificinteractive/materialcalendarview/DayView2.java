...
class DayView2 extends LinearLayout {
    ...
    private TextView tvSolar,tvLunar;

    public DayView2(Context context, CalendarDay day) {
        super(context);
...
//        (2,2,2,2);
        ...
        tvSolar.setGravity(Gravity.CENTER);
        tvLunar = new TextView(context);
        tvLunar.setGravity(Gravity.CENTER);
        addView(tvSolar);
        addView(tvLunar);
        setDay(day);
    }
    ...
    public void setDay(CalendarDay date) {
        this.date = date;
        tvSolar.setText(getLabel());
        tvLunar.setText(getLunarLable());
    }
    ...
    public void setDayFormatter(DayFormatter formatter) {
        ...
        CharSequence currentLabel1 = tvSolar.getText();
        Object[] spans1 = null;
        if (currentLabel1 instanceof Spanned) {
            spans1 = ((Spanned) currentLabel1).getSpans(0, currentLabel1.length(), Object.class);
        }
        SpannableString newLabel1 = new SpannableString(getLabel());
        if (spans1 != null) {
            for (Object span : spans1) {
                newLabel1.setSpan(span, 0, newLabel1.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
        }
        tvSolar.setText(newLabel1);

        CharSequence currentLabel2 = tvLunar.getText();
        Object[] spans2 = null;
        if (currentLabel2 instanceof Spanned) {
            spans2 = ((Spanned) currentLabel2).getSpans(0, currentLabel2.length(), Object.class);
        }
        SpannableString newLabel2 = new SpannableString(getLunarLable());
        if (spans2 != null) {
            for (Object span : spans2) {
                newLabel2.setSpan(span, 0, newLabel2.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
        }
        tvLunar.setText(newLabel2);
    }
    ...
    private void setEnabled() {
        ...
        if (!isInMonth && shouldBeVisible) {
            tvSolar.setTextColor(tvSolar.getTextColors().getColorForState(
                    new int[]{-android.R.attr.state_enabled}, Color.GRAY));
            tvLunar.setTextColor(tvLunar.getTextColors().getColorForState(
                    new int[]{-android.R.attr.state_enabled}, Color.GRAY));
        }
...
//        API 22. Technically harmless to leave on for API 21 and 23, but not worth risking for 23+
    ...
    void applyFacade(DayViewFacade facade) {
        ...
        if (!spans.isEmpty()) {
            SpannableString formattedSolarLabel = new SpannableString(getLabel());
            for (DayViewFacade.Span span : spans) {
                formattedSolarLabel.setSpan(span.span, 0, getLabel().length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
            tvSolar.setText(formattedSolarLabel);
            SpannableString formattedLunarLabel = new SpannableString(getLunarLable());
            for (DayViewFacade.Span span : spans) {
                formattedLunarLabel.setSpan(span.span, 0, getLabel().length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
            tvLunar.setText(formattedLunarLabel);
        }
    ...
    public void setTextAppearance(Context context, int taId) {
        tvSolar.setTextAppearance(getContext(), taId);
        tvLunar.setTextAppearance(getContext(), taId);
        tvLunar.setTextSize(10);
    }
